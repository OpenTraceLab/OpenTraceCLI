...
range_fail:
			...
			if (names[1]) {
				/* Rename channel. */
				otc_dev_channel_name_set(ch, names[1]);
			}
			channellist = g_slist_append(channellist, ch);
...
int parse_driver(char *arg, struct otc_dev_driver **driver, GSList **drvopts)
{
	...
	drvname = g_strdup(g_hash_table_lookup(drvargs, "opentrace_key"));
	g_hash_table_remove(drvargs, "opentrace_key");
	*driver = NULL;
